...
use crate::{
    mjSpec, mjsElement, mjsBody, mjsFrame, mjsDefault,
    mjtObj,
};
...
Return body’s next child of the same type; return `None` if child is last. If recurse is nonzero, also search the body’s subtree.
...
/* mjSpec* mjs_getSpec(mjsElement* element); */
pub fn mjs_getSpec(element: &mut mjsElement) -> Option<&mut mjSpec> {
...
/// Find spec (model asset) by name.
/* mjSpec* mjs_findSpec(mjSpec* spec, const char* name); */
pub fn mjs_findSpec<'spec>(spec: &'spec mut mjSpec, name: &str) -> Option<&'spec mut mjSpec> {
    let name = std::ffi::CString::new(name).expect("`name` must not contain null bytes");
    let ptr = unsafe { crate::bindgen::mjs_findSpec(spec, name.as_ptr()) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr }) }
}
...
pub fn mjs_findBody<'spec>(spec: &'spec mut mjSpec, name: &str) -> Option<&'spec mut mjsBody> {
    let name = std::ffi::CString::new(name).expect("`name` must not contain null bytes");
    let ptr = unsafe { crate::bindgen::mjs_findBody(spec, name.as_ptr()) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr }) }
}
...
pub fn mjs_findElement<'spec>(
    ...
    let name = std::ffi::CString::new(name).expect("`name` must not contain null bytes");
    let ptr = unsafe { crate::bindgen::mjs_findElement(spec, type_, name.as_ptr()) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr }) }
}
...
pub fn mjs_findFrame<'spec>(spec: &'spec mut mjSpec, name: &str) -> Option<&'spec mut mjsFrame> {
    let name = std::ffi::CString::new(name).expect("`name` must not contain null bytes");
    let ptr = unsafe { crate::bindgen::mjs_findFrame(spec, name.as_ptr()) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr }) }
}
...
pub fn mjs_findDefault<'spec>(
    ...
    let ptr = unsafe { crate::bindgen::mjs_findDefault(spec, classname.as_ptr()) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr }) }
}
...
pub fn mjs_getSpecDefault<'spec>(spec: &'spec mut mjSpec) -> Option<&'spec mut mjsDefault> {
    let ptr = unsafe { crate::bindgen::mjs_getSpecDefault(spec) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr })}
}
...
pub fn mjs_firstElement<'spec>(
    ...
    let ptr = unsafe { crate::bindgen::mjs_firstElement(spec, type_) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr }) }
}
...
pub fn mjs_nextElement<'spec>(
    ...
    let ptr = unsafe { crate::bindgen::mjs_nextElement(spec, element) };
    if ptr.is_null() {None} else {Some(unsafe { &mut *ptr }) }
}
